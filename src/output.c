...
/*   Updated: 2019/12/14 17:53:42 by lelajour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		**list_ant_fill(t_room *room, t_end *end, t_slc *slc)
{
		(void)room;
	int		**path;
	int		ant;
	...
	while (++ant < end->nb_ant)
	{
		...
		ft_imemcpy(path[ant] + turn , slc->path[tmp] + 1, slc->len_path[tmp] - 1);
		turn += get_turn(end);
		if (turn > end->lul)
...
void	affichage(t_room *room, t_end *end, t_slc *slc)
{
	...
	move = list_ant_fill(room, end, slc);
